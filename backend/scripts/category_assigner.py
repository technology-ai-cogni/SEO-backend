...
#             {"project_name": domain},
#         )
# # In scripts/category_assigner.py

# def fetch_and_derive_candidate(keyword, top3):
#     """Phase A -- everything that does NOT touch existing categories.
#     Safe to run for many keywords concurrently. `top3` is passed in
#     already-fetched (the SERP fetch itself stays in hosted_categorize.py)."""
...
#     return keyword, candidate_name, titles


# def match_and_save_candidate(domain, candidate_name, titles):
#     """Phase B -- the only part that must run strictly in order (reads +
#     writes the shared per-project category list). This RESTORES the
#     matching steps categorize_from_top3() was supposed to run per this
#     module's docstring but never actually called -- that's the real
#     reason near-duplicate categories (Schools Fees Admissions / Schools
#     Fees Review / Schools Admissions / Schools Fees Curriculum) kept
#     getting created as separate categories instead of merging."""
#     if candidate_name is None:
#         return None

#     existing_category_names = db.list_category_names(domain)

#     # Cheap pre-filter: only compare against categories sharing >=2
#     # significant words (reuses _cluster_significant_words which still
#     # exists in category_checker.py).
#     def _shares_two(a, b):
#         return len(category_checker._cluster_significant_words(a) & category_checker._cluster_significant_words(b)) >= 2

#     plausible_matches = [
#         name for name in existing_category_names
#         if _shares_two(candidate_name, name)
#     ]

#     if not plausible_matches:
#         final_name = candidate_name
#     else:
#         # 1. Deterministic singular/plural dedup -- always resolves to
#         #    whichever spelling is ALREADY stored.
#         plural_match = resolve_plural_or_existing_category(candidate_name, plausible_matches)
#         if plural_match is not None:
#             final_name = plural_match
#         else:
#             # 2. Strict pass: same specific topic AND content type.
#             strict_match = category_checker.find_matching_category(
#                 candidate_name, titles, plausible_matches
#             )
#             if strict_match is not None:
#                 final_name = strict_match
#             else:
#                 # 3. Lenient pass: same core subject/intent, minor
#                 #    modifiers generalized away.
#                 generalized_match = find_matching_category_generalized(
#                     candidate_name, titles, plausible_matches
#                 )
#                 final_name = generalized_match if generalized_match is not None else candidate_name
...
#     """Single-keyword convenience wrapper -- kept for main()/any other
#     caller still wanting the old one-keyword-at-a-time interface. New
#     batch code should call fetch_and_derive_candidate/
#     match_and_save_candidate directly (see hosted_categorize.py)."""
#     _, candidate_name, titles = fetch_and_derive_candidate(keyword, top3)
#     return match_and_save_candidate(domain, candidate_name, titles)
...
def categorize_batch_with_llm(batch_data, existing_categories):
    """
    Takes a batch of keywords and their top-3 titles, plus any existing categories.
    Asks the LLM to process them holistically and assign a category to each,
    reusing existing categories where they match the topic exactly, or generating
    new concise, descriptive categories (max 3 words, no locations, no filler).
    Returns a dictionary mapping: { "keyword": "CategoryName" }
    """
    if not batch_data:
        return {}
    ...
    # Format the existing categories for the prompt
    ...
    for d in batch_data:
        ...
        items.append(f"Keyword: '{kw}'\nFeatures: {features}")
    batch_text = "\n\n".join(items)
    
    system_prompt = (
        "You are an expert SEO categorizer. You are processing a batch of keywords "
        "and their most frequent top topics/features extracted from search results. "
        "Your job is to assign a concise, descriptive category (MAXIMUM 3 words) to each keyword.\n\n"
        "RULES:\n"
        "1. DEDUPLICATION: You MUST reuse existing categories (or use the exact same new category) "
        "for keywords that share the exact same specific topic and search intent. "
        "Don't create 'Best Schools' and 'Top Schools' -- put them in the same category.\n"
        "2. NO LOCATIONS: Never include a city, state, country, or region in the category name.\n"
        "3. NO BEST/TOP: Do NOT add 'Best' or 'Top' to the category name unless the feature says (Best/Top applied: Yes).\n"
        "4. NO METADATA/FILLER: Never include words like 'guide', 'reviews', 'child', 'insights'.\n"
        "5. ENTITY TYPE: Pick ONE entity type (e.g. 'Schools' or 'Agencies') and stick to it.\n\n"
        "OUTPUT FORMAT:\n"
        "You must return ONLY a raw JSON object containing a dictionary under the key 'mappings'. "
        "The dictionary must map each keyword to its assigned category string. "
        "Example:\n"
        "{\n"
        '  "mappings": {\n'
        '    "keyword 1": "Category A",\n'
        '    "keyword 2": "Category A",\n'
        '    "keyword 3": "Category B"\n'
        "  }\n"
        "}"
    )
    ...
    try:
        ...
        return result_json.get("mappings", {})
    except Exception as e:
        print(f"[categorize_batch_with_llm] Error: {e}")
        return {}
