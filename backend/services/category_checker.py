...
#     "bhopal", "nagpur", "patna", "ncr", "navi", "airoli", "thane",
#     "kukatpally", "bannerghatta",
...
#     results = []
#     if not html:
#         pass # Will fall back to Firecrawl below
...
#             # Combine top_3 and other to get up to 3
...
# def _common_words_across_titles(titles, min_titles=2, keyword=None):
#     """Words appearing in at least `min_titles` of the given titles
#     (case-insensitive). Words with frequency >= 2 across SERP titles
#     AND present in the keyword are prioritized as mandatory core words."""
#     required = min(min_titles, len(titles)) if titles else min_titles

#     serp_titles = titles[:3] if len(titles) >= 3 else titles
#     serp_doc_presence = Counter()
#     surface_counts = {}
...
#             serp_doc_presence[norm] += 1

#     doc_presence = Counter(serp_doc_presence)
#     kw_norms = set()
#     if keyword:
#         for w in _title_word_set(keyword):
#             norm = _singularize_word(w)
#             kw_norms.add(norm)
#             doc_presence[norm] += 1
#             if norm not in surface_counts:
#                 surface_counts.setdefault(norm, Counter())[w] += 1
...
#     # Mandatory words: appears >= 2 times in SERP titles AND is present in keyword
#     mandatory_norms = {
#         norm for norm in qualifying_norms
#         if serp_doc_presence[norm] >= 2 and norm in kw_norms
#     }
...
#     # First add mandatory words (in keyword order)
#     if keyword:
#         for w in re.findall(r"[A-Za-z0-9]+", keyword.lower()):
#             norm = _singularize_word(w)
#             if norm in mandatory_norms and norm not in seen:
#                 seen.add(norm)
...
#     """When Best/Top applies, prefix the category with 'Best/Top ' and
#     keep only the TOP 2 remaining words (capped -- not every common word),
#     taken in the order they already appear in candidate_name. Singular vs.
#     plural is NEVER forced here: whatever plurality the kept words
#     naturally had in the source titles is preserved exactly as found -- no
#     blind grammar-rule pluralization that could mangle a mass noun like
...
#     """
#     Build a category name purely deterministically from the words common
#     to at least 2 of the 3 fetched titles (see _common_words_across_titles
#     for the exact matching / singular-plural / location-exclusion rules).
#     NO LLM call is involved -- every qualifying word is used, in the order
#     it first appears across the titles, joined into one phrase.
...
#     If nothing is shared by 2+ titles, falls back to the first title's
#     own words (deduped by singular/plural form, still excluding
#     location/stopword/ranking words and pure digits).

#     The Best/Top rule (including its top-2-word cap) is applied
#     separately, deterministically, after this function returns.
#     """
#     count_documents = titles + [keyword] if keyword else titles
#     qualifying_words = _common_words_across_titles(titles, min_titles=2, keyword=keyword)
...
#     candidate = " ".join(w.capitalize() for w in qualifying_words)
#     return _ensure_plural_category_name(_enforce_single_entity_type(_clean_category_text(candidate), count_documents))
...
# _KNOWN_SINGULAR_ENTITIES = {
#     "school": "Schools", "college": "Colleges", "agency": "Agencies",
#     "company": "Companies", "firm": "Firms", "service": "Services",
#     "provider": "Providers", "fee": "Fees", "admission": "Admissions",
#     "course": "Courses", "class": "Classes", "institute": "Institutes",
#     "university": "Universities", "hospital": "Hospitals", "clinic": "Clinics",
#     "center": "Centers", "centre": "Centres", "store": "Stores", "shop": "Shops",
#     "office": "Offices", "bank": "Banks", "agent": "Agents", "broker": "Brokers",
#     "developer": "Developers", "contractor": "Contractors", "consultant": "Consultants"
# }


# def _ensure_plural_category_name(category_name: str) -> str:
...
#     for w in words:
#         w_lower = w.lower()
#         if w_lower in _KNOWN_SINGULAR_ENTITIES:
#             plural_form = _KNOWN_SINGULAR_ENTITIES[w_lower]
#             if w.isupper():
#                 new_words.append(plural_form.upper())
#             elif w.istitle():
#                 new_words.append(plural_form.capitalize())
#             else:
#                 new_words.append(plural_form.lower())
#         else:
#             new_words.append(w)
#     return " ".join(new_words)


# _RAW_CANDIDATE_CACHE = {}

# def _refine_category_name_with_llm(raw_candidate: str, keyword: str) -> str:
#     """Use OpenAI to refine awkward combinations into a clean, meaningful SEO category.
#     Strips out specific business/school names (like 'D.A.V.') or stray places."""
#     if raw_candidate in _RAW_CANDIDATE_CACHE:
#         return _RAW_CANDIDATE_CACHE[raw_candidate]

#     if not OPENAI_API_KEY:
#         return _ensure_plural_category_name(raw_candidate)
#     try:
#         client = get_openai_client()
#         prompt = (
...
#             "CRITICAL RULES:\n"
#             "1. NEVER include question words (e.g. 'which', 'what', 'how') or filler metadata words (e.g. 'child', 'parents', 'insights').\n"
#             "2. REMOVE any specific brand name, business name, or school name (e.g. 'D.A.V.', 'EuroSchool', 'Horizon'). The category must be generic (e.g. 'Public Schools CBSE').\n"
#             "3. REMOVE any specific city, state, or neighborhood name (e.g. 'Navi', 'Mumbai', 'Airoli').\n"
#             "4. Retain all specific topic modifiers (e.g. 'ICSE', 'CBSE', 'international', 'digital', 'marketing').\n"
#             "5. ALWAYS use the PLURAL form of subject/entity nouns. For example: 'school' -> 'schools', 'agency' -> 'agencies', 'company' -> 'companies', 'college' -> 'colleges', 'service' -> 'services', 'firm' -> 'firms', 'provider' -> 'providers'. If the word is already plural, keep it plural.\n"
#             "6. MANDATORY INCLUSION: If a concept word appears 2+ times in the fetched page titles AND is also present in the keyword (e.g. 'schools', 'cbse', 'international'), you MUST include it in the category name unless it is a location or brand name.\n"
#             "Keep it concise (maximum 3 words). Do not add 'Best' or 'Top' (that is handled separately).\n"
...
#         final_refined = _ensure_plural_category_name(refined if refined else raw_candidate)
        
#         _RAW_CANDIDATE_CACHE[raw_candidate] = final_refined
...
def _brightdata_fetch(target_url):
    ...
    for attempt in range(1, MAX_REQUEST_RETRIES + 1):
        try:
            ...
            last_error = f"Network Timeout / Connection Error (Attempt {attempt}/{MAX_REQUEST_RETRIES}): {e}"
            if attempt < MAX_REQUEST_RETRIES:
                time.sleep(RETRY_BACKOFF_SECONDS * (2 ** (attempt - 1)))
        except requests.exceptions.HTTPError as e:
            status = e.response.status_code if e.response is not None else None
            last_error = f"HTTP Error {status} (Attempt {attempt}/{MAX_REQUEST_RETRIES}): {e}"
            if status in (429, 500, 502, 503, 504) and attempt < MAX_REQUEST_RETRIES:
                time.sleep(RETRY_BACKOFF_SECONDS * (2 ** (attempt - 1)))
            else:
                break
    ...
    if "application/json" in content_type:
        try:
            ...
            print(f"[Bright Data Error] Failed to parse JSON response body for URL '{target_url}': {ex}")
            return None
    else:
        html = resp.text
...
def get_top3_for_category(keyword, country_code=None):
    ...
    gl = country_code or COUNTRY_CODE
    search_url = f"https://{GOOGLE_DOMAIN}/search?q={quote(keyword)}&gl={gl}&hl={LANGUAGE_CODE}"
    print(f"[SERP Fetch START] Keyword: '{keyword}' | Country: '{gl}'")
    html = _brightdata_fetch(search_url)
    results = []
    if not html:
        print(f"[SERP Fetch WARNING] Bright Data returned NO HTML for keyword: '{keyword}'")
    else:
        soup = BeautifulSoup(html, "html.parser")
        container = soup.find("div", id="rso") or soup.find("div", id="search")
        ...
        # Fallback to "dURPMd" class container if no results found in main containers
        if not results:
            durpmd_elements = soup.find_all("div", class_="dURPMd")
            if durpmd_elements:
                print(f"[SERP Fetch DEBUG] Main container empty for '{keyword}'. Trying fallback container '.dURPMd'...")
                for element in durpmd_elements:
        ...
        if not results:
            print(f"[SERP Fetch WARNING] Bright Data HTML parsed but 0 organic results found for '{keyword}' (Possible CAPTCHA or no organic rankings).")

    if not results:
        # Fallback to Firecrawl
        try:
            print(f"[SERP Fetch FALLBACK] Triggering Firecrawl fallback for keyword: '{keyword}'...")
            from scripts.firecrawl_scraper import fetch_top_results_via_firecrawl
            fc_data = fetch_top_results_via_firecrawl(keyword, country_code=gl)
            combined = fc_data.get("top_3", []) + fc_data.get("other", [])
            results = combined[:5]
        except Exception as e:
            print(f"[Firecrawl Error] Fallback failed for keyword '{keyword}' | Reason: {e}")

    if results:
        print(f"[SERP Fetch SUCCESS] Extracted {len(results)} results for keyword: '{keyword}':")
        for idx, item in enumerate(results, start=1):
            print(f"   Rank #{idx}: Title=\"{item.get('title')}\" | URL={item.get('url')}")
    else:
        print(f"[SERP Fetch FAILED] ❌ Could NOT fetch SERP data for keyword: '{keyword}' after all attempts.")
...
def derive_category_name(titles, keyword=None):
    ...
    candidate = " ".join(w.capitalize() for w in qualifying_words)
    return _enforce_single_entity_type(_clean_category_text(candidate), titles)
...
TARGET_TYPE_LANDING = "Landing Page"
TARGET_TYPE_BLOG = "Blogs"
TARGET_TYPE_TOPICAL = "Topical Blogs"
_TARGET_TYPE_TIE_BREAK_PRIORITY = [TARGET_TYPE_TOPICAL, TARGET_TYPE_BLOG, TARGET_TYPE_LANDING]
...
def categorize_keyword(keyword, domain, country_code=None, pre_fetched_top3=None):
    """
    Category-only pipeline for one keyword, scoped to `domain`: fetch
    top-3 titles (from the given `country_code`'s Google region, or the
    .env default if not given), derive a candidate category name, apply
...
def _refine_category_name_with_llm(raw_candidate: str, keyword: str) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            "3. REMOVE any specific city, state, or neighborhood name (e.g. 'Navi', 'Mumbai', 'Airoli').\n"
            "4. Retain all specific topic modifiers (e.g. 'ICSE', 'CBSE', 'international', 'digital', 'marketing').\n"
            "Keep it concise (MAXIMUM 2 WORDS). Ensure the category is meaningful, professional, and clean (e.g. 'CBSE Schools', 'International Schools', 'Digital Marketing'). Do not add 'Best' or 'Top' (that is handled separately).\n"
            "Output ONLY the refined category name, nothing else."
        )
        resp = client.chat.completions.create(
            model=OPENAI_CHAT_MODEL,
        ...
        refined = resp.choices[0].message.content.strip()
        refined = refined.strip("\"'")
        final_refined = refined if refined else raw_candidate
